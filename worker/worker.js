...
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      //   3) Google chặn 429/timeout → fallback Groq AI (xoay key sẵn có,
      //      prompt bắt buộc chỉ trả chuỗi Romaji Hepburn).
      //   4) Không cache kết quả rỗng để lần sau có thể thử lại.
      ...
      if (path === "/api/romaji") {
        ...
        // 3) Fallback Groq AI khi Google chặn (429) hoặc timeout
        if (!romaji) {
          try {
