...
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // Helper: Innertube get_transcript API
      async function fetchInnertubeTranscript(videoId, headers) {
        // Step 1: Get params from player response
        ...
        for (const pat of [
          ...
          if (m) { try { playerResponse = JSON.parse(m[1]); break; } catch(e) {} }
        }
        ...
        // Step 2: Call Innertube get_transcript
        const innertubeBody = {
          context: {
            client: {
              clientName: "WEB",
          ...
          params: jaTrack.baseUrl.split('params=')[1]?.split('&')[0]
        };

        if (!innertubeBody.params) {
          // Extract from URL
          const urlObj = new URL(jaTrack.baseUrl);
          innertubeBody.params = urlObj.searchParams.get('params');
        }
        if (!innertubeBody.params) return null;
        ...
        if (!transcriptResp.ok) throw new Error(`Transcript API: ${transcriptResp.status}`);

        const data = await transcriptResp.json();
        const body = data.transcript?.transcriptContentRenderer?.body?.transcriptBodyRenderer?.cueGroups;
        if (!body) return null;
        ...
        for (const group of body) {
          ...
          for (const cue of cues) {
            const c = cue.transcriptCueRenderer;
            if (c?.cue?.simpleText) {
              const startMs = parseInt(c.startOffsetMs) || 0;
              const durMs = parseInt(c.durationMs) || 2000;
              segments.push({
                start: startMs / 1000,
                duration: durMs / 1000,
                end: (startMs + durMs) / 1000,
                text: c.cue.simpleText.trim(),
                lang: jaTrack.languageCode
              });
            }
          }
        }
      ...
      if (path === "/api/yt-caption" && request.method === "POST") {
        try {
          ...
          // ========== METHOD 1: Innertube get_transcript API ==========
          try {
            const innertubeSegments = await fetchInnertubeTranscript(videoId, YT_HEADERS);
          ...
          // ========== METHOD 2: Page scrape + caption fetch ==========
          const pageResponse = await fetch(`https://www.youtube.com/watch?v=${videoId}`, { headers: YT_HEADERS });
          if (!pageResponse.ok) {
          ...
          for (const pattern of patterns) {
            ...
            if (m) { try { playerResponse = JSON.parse(m[1]); break; } catch(e) { continue; } }
          }
          if (!playerResponse) {
